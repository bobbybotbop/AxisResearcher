...
- **Logs tab** (`/logs`): a persistent audit trail of backend actions. `backend/logger.py` appends entries to `logs.ndjson` in the project root (capped at 1000). `GET /api/logs` reads and returns them newest-first. The tab has filter chips by event type and click-to-navigate to a specific SKU in the History tab (sets `uploadListingsSearch` then navigates to `/history`). Event types: `draft_created`, `draft_completed`, `upload`, `restock` (with `source: "manual"|"auto"`), `import`.
