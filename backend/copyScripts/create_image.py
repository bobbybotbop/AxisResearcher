...
from backend.copyScripts.imageEditing import downscale_image_bytes
...
def generate_image_from_urls(
    ...
    Args:
        ...
        prompt_modifier (str, optional): For ANGLE_VARIANT: the angle string to substitute into
            the template. For other types: additional text to append to the prompt.
    ...
    # Always append extra_instructions after all other prompt construction
    ...
    for image_url in image_urls:
        ...
        if image_url.startswith("/api/bg-removed-image/"):
            filename = image_url.split("/api/bg-removed-image/", 1)[1]
            filepath = project_root / "generated-images" / filename
            try:
                with open(filepath, "rb") as f:
                    raw = f.read()
                encoded = base64.b64encode(downscale_image_bytes(raw)).decode("utf-8")
                resolved_url = f"data:image/png;base64,{encoded}"
                print(f"📎 Inlined local image as base64: {filename}")
            except Exception as e:
                print(f"⚠️ Could not read local image {filepath}: {e}")
        elif image_url.startswith("http"):
            try:
                img_resp = requests.get(image_url, timeout=15)
                img_resp.raise_for_status()
                encoded = base64.b64encode(downscale_image_bytes(img_resp.content)).decode("utf-8")
...
def _extension_from_mime(mime_type: str) -> str:
    """Map a MIME type to a file extension for saved outputs."""
    if not mime_type:
        return ".png"
    m = mime_type.lower()
...
def _load_latest_api_response_json():
    """Load the newest JSON in api-responses. Returns (dict, Path) or None."""
...
def _openrouter_response_dict_to_image_bytes_and_mime(result):
    """
    Parse an OpenRouter-style API result dict and return (image_bytes, mime_type) or None.
    Same parsing rules as the former decode_image_from_response / upload_image_to_ebay duplicate.
    """
    image_data = None
    image_url = None
    is_base64 = False
    ...
    try:
        if is_base64 and image_data:
            ...
            try:
                ...
                print(f"❌ Error decoding base64 data: {e}")
                return None
            mime_out = mime_type or "image/png"
            return image_bytes, mime_out
        if image_url:
            ...
            image_bytes = img_response.content
            content_type = img_response.headers.get("content-type", "")
            mime_out = content_type.split(";", 1)[0].strip() if content_type else "image/jpeg"
            return image_bytes, mime_out
...
def decode_image_from_response():
    ...
    """
    try:
        loaded = _load_latest_api_response_json()
        if not loaded:
            return None
...
def upload_image_to_ebay(picture_name="Uploaded Image"):
    ...
    if not loaded:
        return None
    result, _json_path = loaded
    parsed = _openrouter_response_dict_to_image_bytes_and_mime(result)
    if not parsed:
        return None
    image_bytes, mime_type = parsed
    return upload_image_bytes_to_ebay(image_bytes, mime_type, picture_name)
...
def categorize_image(image_url, model="bytedance-seed/seed-1.6-flash"):
    ...
    # Fetch and downscale the image to max 1024px before sending to the API
    resolved_url = image_url
    if image_url.startswith("http"):
        try:
            ...
            print(f"⚠️ Could not fetch/downscale image for categorization, passing URL directly: {e}")
